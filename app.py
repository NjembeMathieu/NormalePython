...
def show_api_section(predictor, api_client):
    ...
    with st.expander("Endpoint: GET /health"):
        st.markdown("""
        **Vérifie le statut de l'API et le modèle chargé**
    ...
    with st.expander("Endpoint: POST /predict"):
        st.markdown("""
        **Prédiction unique**
    ...
    with st.expander("Endpoint: POST /batch_predict"):
        st.markdown("""
        **Prédictions par lot**
        
        ```bash
        curl -X POST {}/batch_predict \\\\
          -H "Content-Type: application/json" \\\\
          -d '{{
            "records": [
              {{
...
print(response.json())
        """.format(f"{api_url}/predict", f"{api_url}/batch_predict"))
...
curl -X POST {} \\
  -H "Content-Type: application/json" \\
  -d '{{"Gender":1,"Age":45,"Annual Salary":70000,"Credit Card Debt":10000,"Net Worth":400000}}'

# Prédictions par lot
curl -X POST {} \\
  -H "Content-Type: application/json" \\
  -d '{{"records":[{{"Gender":1,"Age":35,"Annual Salary":60000,"Credit Card Debt":8000,"Net Worth":300000}},{{"Gender":0,"Age":50,"Annual Salary":80000,"Credit Card Debt":12000,"Net Worth":500000}}]}}'
        """.format(f"{api_url}/predict", f"{api_url}/batch_predict"))
